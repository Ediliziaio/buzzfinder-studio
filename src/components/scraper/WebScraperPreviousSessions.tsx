...
export function WebScraperPreviousSessions({ sessions, onLoad }: Props) {
  const [open, setOpen] = useState(false);

  const websiteSessions = sessions.filter((s) => s.tipo === "website");
  if (websiteSessions.length === 0) return null;
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-md border border-border bg-card px-3 py-2 text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
        <span>Sessioni precedenti ({websiteSessions.length})</span>
        <ChevronDown className={`h-3 w-3 transition-transform ${open ? "rotate-180" : ""}`} />
      </CollapsibleTrigger>
      <CollapsibleContent className="mt-2 space-y-2">
        {websiteSessions.map((s) => (
          <div
            key={s.id}
